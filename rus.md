...
50×50, это серьезная проблема. Браузеру все равно приходится загружать
...
реализовать все с помощью CSS, стоит задуматься о том, чтобы использовать эту
...
картинки наверху несложно заметить. Для загрузки этих значков пришлось бы выполнить 
или 8 HTTP-запросов, или всего один, и лучшим решением было бы использовать один
...
вручную][10], а не использовать инструменты, создающие их за вас.
...
*Будьте осторожны при уменьшении размеров CSS-файлов, так как ваши 
медиа-выражения могут сломаться.*
...
загрузит, разархивирует и проведет анализ сжатого файла так же, как любого
...
приводит к существенному увеличению времени отрисовки страницы на экране, а
также, если CSS описывает стили элемента, расположенного выше в
HTML-документе, браузеру необходимо перерисовать содержимое экрана
